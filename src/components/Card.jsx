...
Card.Description = function CardDescription({ children }) {
  return (
    <p className="relative z-10 mt-2 text-sm text-slate-700 dark:text-slate-400">
      {children}
    </p>
  )
}
...
Card.Cta = function CardCta({ children }) {
  return (
    <div
      aria-hidden="true"
      className="relative z-10 mt-4 flex items-center text-sm font-medium text-emerald500"
    >
      {children}
...
Card.Eyebrow = function CardEyebrow({
  ...
  return (
    <Component
      className={clsx(
        className,
        'relative z-10 order-first mb-3 flex items-center text-sm text-slate-500 dark:text-slate-500',
        decorate && 'pl-3.5'
      )}
      {...props}
    >
