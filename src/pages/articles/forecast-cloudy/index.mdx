...
I used Ubuntu server, so I installed using Caddy's [apt repository](https://caddyserver.com/docs/install#debian-ubuntu-raspbian). It installs the binary, creates a system service and starts the server. Now's a good time to go to your domain registrar and add a domain or subdomain to use for your controller. I picked the creative and unique domain `code.markobodiroza.com` because I like to be original. I created a DNS A record that points to the public IP address of my EC2 instance. We can continue with the installation while DNS records propagate across the internets.
...
CODER_ACCESS_URL=https://code.markobodiroza.com
CODER_ADDRESS=
